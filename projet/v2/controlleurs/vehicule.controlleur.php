...
function show_ajout_voiture($nbre=1,$get=null){
  ...
  $categories=find_all_categorie();
  $type_vehicule=find_all_type_vehicule();
  $model=find_all_modele_by_id((int)isset($_SESSION['post']['modele']) ? $_SESSION['post']['modele'] : 0);
  $marque=find_marque_by_id((int)isset($_SESSION['post']['marque']) ? $_SESSION['post']['marque'] : 0);
  $categorie=find_categorie_by_id((int)isset($_SESSION['post']['categorie']) ? $_SESSION['post']['categorie'] : 0);
  $voiture=find_vehicule_by_id((int)$get);
  $image = find_all_image_vehicule_by_id_((int)isset($_GET['id_vehicule']) ? $_GET['id_vehicule'] : 0);
...
function show_liste_categorie(){
  if (isset($_POST['archiver'])) {
    ...
    $etat='normal';
    archive_categorie($etat,(int)isset($_POST['id_categorie']) ? $_POST['id_cateogire'] : 0);
  }
  $categories=find_all_categorie();
  $nbrPage=5;
...
function show_liste_modele(){
  if (isset($_POST['archiver'])) {
    ...
    archive_modele('normal',(int)isset($_POST['id_modele']) ? $_POST['id_modele'] : 0);
  }
  $modeles=find_all_modele();
  $nbrPage=5;
...
function show_liste_option(){
  if (isset($_POST['archiver'])) {
    ...
    archive_option('normal',(int)isset($_POST['id_option_vehicule']) ? $_POST['id_option_vehicule'] : 0);
  }
  $options=find_all_option();
  $nbrPage=5;
