<?php
   ...
   /*  function find_all_categorie_par_page($premier,$parPage):array{
    ...
    function find_all_categorie($start=null,$parPage=null):array{
        ...
        $pdo = ouvrir_connection_db(); // Ouverture de la connexion

        // Construction de la requête SQL
        $sql = "SELECT * FROM categorie";

        // Vérification et ajout des conditions
        if (!is_null($start) && !is_null($parPage)) {
            $sql .= " LIMIT $start, $parPage";
        } elseif (!is_null($start)) {
            // Si $start est défini mais $parPage ne l'est pas
            $sql .= " LIMIT $start, 10"; // Par exemple, 10 est une valeur arbitraire pour le nombre de lignes par défaut
        } elseif (!is_null($parPage)) {
            // Si $parPage est défini mais $start ne l'est pas
            $sql .= " LIMIT $parPage"; // Seulement $parPage
        }
        ...
        fermer_connection_db($pdo); // Fermeture de la connexion
        return $datas;
    }
    ...
    function find_all_modele($start=null,$parPage=null):array{
        ...
        // Vérification et ajout des conditions pour la clause LIMIT
        if (!is_null($start) && !is_null($parPage)) {
            $sql .= " LIMIT $start, $parPage";
        } elseif (!is_null($start)) {
            ...
            // Si seulement $parPage est défini
            $sql .= " LIMIT $parPage"; // Seulement $parPage
        }
